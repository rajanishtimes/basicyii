...
class LoggableBehavior extends Behavior
{
	...
	public $ignoredClasses = array('common\models\Constants','common\models\CustomScoreLog');
	...
	public function auditAttributes($insert, $newattributes, $oldattributes = array())
	{
		...
		foreach ($newattributes as $name => $value) {
			...
			if ($value != $old) {
                ...
                //\Yii::trace(print_r($oldValueArray,true),__METHOD__);
                //\Yii::trace(print_r($newValueArray,true),__METHOD__);
                
                if(count($oldValueArray)>0)
                {
                 //$this->leaveTrail($insert ? 'SET' : 'CHANGE', $name, $value, $old);
                 $this->leaveTrail($insert ? 'SET' : 'CHANGE', serialize($nameArray), serialize($newValueArray), serialize($oldValueArray));
                }
