...
class DynamicField extends InputWidget
{
    ...
    public function run() {
        ...
        $models = ArrayHelper::getValue($this->options, 'models');
         ...
         //\Yii::trace("render done problem mod".print_r($models,true),__METHOD__);
        ...
        //\Yii::trace("render done problem data count".print_r($data_count,true),__METHOD__);
        ...
        if($data_count && is_array($models) && count($models))
          {
            ...
            //\Yii::trace("render done problem multiple ",__METHOD__);
            $count = 0;
            foreach($models as $singleModel)
            {
               ...
               // \Yii::trace("render done problem 123".print_r($singleModel,true),__METHOD__);
               // \Yii::trace("render done problem 456".print_r($this->options,true),__METHOD__);   
                $this->data[][$this->name] = $singleModel;
                $count++;
            }
            //\Yii::trace("render done problem mod tt".print_r($this->data,true),__METHOD__);
            //\Yii::trace("render done problem ",__METHOD__);
        }else{ 
            $singleModel = ArrayHelper::getValue($this->options, 'model');
           // \Yii::trace("render done problem single ".print_r($singleModel,true),__METHOD__);
            unset($this->options['model']);
            unset($this->options['models']);
        }
         //\Yii::trace("render done problem mod tt................".print_r($singleModel,true),__METHOD__);
        
        $this->options['value'] = '';
        echo Html::beginTag('div', ['class'=>  'input-group']);
        echo $this->getInput($this->type, $this->list,$model);
        $this->generateButton();
