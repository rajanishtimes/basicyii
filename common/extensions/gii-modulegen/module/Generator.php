...
class Generator extends \yii\gii\Generator
{
    ...
    public $baseControllerClass = 'common\component\AccessController';
...
EOD;
    ...
    {
        return ['module.php', 'appurlasset.php'];
    }
    ...
    public function generateworkflow() {
        ...
        $files[] = new CodeFile($controllerFile, $this->render('workflow/controller.php'));
        
        $controllerFile = Yii::getAlias('@' . str_replace('\\', '/', ltrim($this->tcontrollerClass, '\\')) . '.php');

        $files[] = new CodeFile($controllerFile, $this->render('workflow/tcontroller.php'));

        if (!empty($this->searchModelClass)) {
            $searchModel = Yii::getAlias('@' . str_replace('\\', '/', ltrim($this->searchModelClass, '\\') . '.php'));
            $files[] = new CodeFile($searchModel, $this->render('workflow/search.php'));
        }
        
        if (!empty($this->tsearchModelClass)) {
            $searchModel = Yii::getAlias('@' . str_replace('\\', '/', ltrim($this->tsearchModelClass, '\\') . '.php'));
            $files[] = new CodeFile($searchModel, $this->render('workflow/tsearch.php'));
        }

        $viewPath = $this->getViewPath();
        $tviewPath = $this->getViewPath(true);
        $templatePath = $this->getTemplatePath() . '/workflow/views';
        foreach (scandir($templatePath) as $file) {
            if (empty($this->searchModelClass) && $file === '_search.php') {
                continue;
            }
            if (is_file($templatePath . '/' . $file) && pathinfo($file, PATHINFO_EXTENSION) === 'php') {
                $files[] = new CodeFile("$viewPath/$file", $this->render("workflow/views/$file"));
            }
        }
		
		$templatePath = $this->getTemplatePath() . '/workflow/tviews';
		foreach (scandir($templatePath) as $file) {
            if (empty($this->tsearchModelClass) && $file === '_search.php') {
                continue;
            }
            if (is_file($templatePath . '/' . $file) && pathinfo($file, PATHINFO_EXTENSION) === 'php') {
                $files[] = new CodeFile("$tviewPath/$file", $this->render("workflow/tviews/$file"));
            }
        }
    ...
    public function generatewithoutworkflow(){
        $controllerFile = Yii::getAlias('@' . str_replace('\\', '/', ltrim($this->controllerClass, '\\')) . '.php');

        $files[] = new CodeFile($controllerFile, $this->render('withoutworkflow/controller.php'));
        
        $controllerFile = Yii::getAlias('@' . str_replace('\\', '/', ltrim($this->tcontrollerClass, '\\')) . '.php');

        $files[] = new CodeFile($controllerFile, $this->render('withoutworkflow/tcontroller.php'));

        if (!empty($this->searchModelClass)) {
            $searchModel = Yii::getAlias('@' . str_replace('\\', '/', ltrim($this->searchModelClass, '\\') . '.php'));
            $files[] = new CodeFile($searchModel, $this->render('withoutworkflow/search.php'));
        }
        
        if (!empty($this->tsearchModelClass)) {
            $searchModel = Yii::getAlias('@' . str_replace('\\', '/', ltrim($this->tsearchModelClass, '\\') . '.php'));
            $files[] = new CodeFile($searchModel, $this->render('withoutworkflow/tsearch.php'));
        }

        $viewPath = $this->getViewPath();
        $tviewPath = $this->getViewPath(true);
        $templatePath = $this->getTemplatePath() . '/withoutworkflow/views';
        foreach (scandir($templatePath) as $file) {
            if (empty($this->searchModelClass) && $file === '_search.php') {
                continue;
            }
            if (is_file($templatePath . '/' . $file) && pathinfo($file, PATHINFO_EXTENSION) === 'php') {
                $files[] = new CodeFile("$viewPath/$file", $this->render("withoutworkflow/views/$file"));
            }
        }
		
		$templatePath = $this->getTemplatePath() . '/withoutworkflow/tviews';
		foreach (scandir($templatePath) as $file) {
            if (empty($this->tsearchModelClass) && $file === '_search.php') {
                continue;
            }
            if (is_file($templatePath . '/' . $file) && pathinfo($file, PATHINFO_EXTENSION) === 'php') {
                $files[] = new CodeFile("$tviewPath/$file", $this->render("withoutworkflow/tviews/$file"));
            }
        }
    ...
    public function generate()
    {
        ...
        if($this->isworkflow){
            return array_merge($files,$this->generateworkflow());
        }else{
            return array_merge($files,$this->generatewithoutworkflow());
        }        
    }
